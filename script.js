...
function renderAccountsList() {
    ...
    document.querySelectorAll(".edit-acc").forEach(btn => {
        btn.addEventListener("click", e => {
            ...
            form.querySelector("input[placeholder='First Name']").value = acc.firstName;
...
function setupAccountForm() {
    ...
    form.addEventListener("submit", e => {
        ...
        if (editIdx !== undefined) {
            window.db.accounts[editIdx] = { firstName, lastName, email, password, role, verified };
...
function renderEmployeesTable() {
    ...
    window.db.employees.forEach((emp, idx) => {
        ...
        const dept = window.db.departments.find(d => d.name === emp.department) || {};
        const tr = document.createElement("tr");
        tr.innerHTML = `
            <td>${emp.id}</td>
...
function setupEmployeeForm() {
    ...
    form.addEventListener("submit", e => {
        ...
        if (!userExists) { alert("User email does not exist."); return; }

        const editIdx = form.dataset.editIdx;
        if (editIdx !== undefined) {
            window.db.employees[editIdx] = { id, userEmail, position, department, hireDate };
            delete form.dataset.editIdx;
        } else {
            window.db.employees.push({ id, userEmail, position, department, hireDate });
        }
        saveToStorage();
        renderEmployeesTable();
        form.reset();
